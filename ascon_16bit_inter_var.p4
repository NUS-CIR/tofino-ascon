...
{   
    ...
    action diffusion_4_0 () {
        // ROR(t.x[0], 28);
        @in_hash { meta.q0 = meta.u0[27:12]; }
    }
    action diffusion_5_0 () {
        // ROR(t.x[0], 28);
        @in_hash { meta.q1 = meta.u0[11:0]++meta.u0[63:60]; }
        // ROR(t.x[1], 39);
    }

    action diffusion_6_0 () {
        // ROR(t.x[0], 28);
        @in_hash { meta.q2 = meta.u0[59:44]; }
        // ROR(t.x[1], 39);
    }

    action diffusion_7_0 () {
        // ROR(t.x[0], 28);
        @in_hash { meta.q3 = meta.u0[43:28]; }
        // ROR(t.x[1], 39);
    }
    ...
    action diffusion_12_0() {
        // s->x[0] = t.x[0] ^ ROR(t.x[0], 19) ^ ROR(t.x[0], 28);
        @in_hash { hdr.ascon.s0[63:48] = meta.t0[63:48] ^ meta.p0; } 

    }
    action diffusion_13_0() {
        // s->x[0] = t.x[0] ^ ROR(t.x[0], 19) ^ ROR(t.x[0], 28);
        @in_hash { hdr.ascon.s0[47:32] = meta.t0[47:32] ^ meta.p1; } 

    }

    action diffusion_14_0() {
        // s->x[0] = t.x[0] ^ ROR(t.x[0], 19) ^ ROR(t.x[0], 28);
        @in_hash { hdr.ascon.s0[31:16] = meta.t0[31:16] ^ meta.p2; } 

    }

    action diffusion_15_0() {
        // s->x[0] = t.x[0] ^ ROR(t.x[0], 19) ^ ROR(t.x[0], 28);
        @in_hash { hdr.ascon.s0[15:0] = meta.t0[15:0] ^ meta.p3; } 
