...
//First layer--for obtaining hdr.s0
    action diffusion_0_0 () {
        // ROR(t.x[0], 19)
        // @in_hash { meta.p= meta.t0[18:0] ++ meta.t0[63:32]++meta.t0[31:19];  }
        meta.p= meta.t0[18:0] ++ meta.t0[63:32]++meta.t0[31:19];
        // meta.p[63:32]= meta.t0[18:0] ++ meta.t0[63:51]; 
    }
    ...
    action diffusion_2_0 () {
        // ROR(t.x[0], 28);
        // @in_hash { meta.q = meta.t0[27:0] ++ meta.t0[63:32]++meta.t0[31:28]; }
        meta.q = meta.t0[27:0] ++ meta.t0[63:32]++meta.t0[31:28];
        // meta.q[63:32] = meta.t0[27:0] ++ meta.t0[63:60];
    }
...
//Second layer--for obtaining hdr.s1
    action diffusion_0_1 () {
        // @in_hash { meta.p = meta.t1[60:29] ++meta.t1[28:0]++meta.t1[63:61];        }
        meta.p = meta.t1[60:29] ++meta.t1[28:0]++meta.t1[63:61];     
        // ROR(t.x[1], 61) 
    }
    ...
    action diffusion_2_1 () {
        // @in_hash { meta.q = meta.t1[38:7]++meta.t1[6:0]++meta.t1[63:39]; }
        meta.q = meta.t1[38:7]++meta.t1[6:0]++meta.t1[63:39];
        // ROR(t.x[1], 39);
    }
