...
{   
    ...
    apply {
        ...
        if(hdr.ascon.curr_round==0x24){
            ...
            // if(ig_intr_md.ingress_port[6:0])
            ig_tm_md.ucast_egress_port[8:7] = ig_intr_md.ingress_port[8:7];
            ...
            do_recirculate();
            meta.rnd_bit = rng_2.get();
            if (meta.rnd_bit == 0){
                ig_tm_md.ucast_egress_port[6:0] = 32;
		    } 
            if(meta.rnd_bit == 1){
			    ig_tm_md.ucast_egress_port[6:0] = 24;
            }
            if(meta.rnd_bit == 2){
			    ig_tm_md.ucast_egress_port[6:0] = 48;
            }
            if(meta.rnd_bit == 3){
			    ig_tm_md.ucast_egress_port[6:0] = 40;
            }
        }
    }
}
