...
#define eg_port 56; //fow h/w needs to be 0x8
...
{   
    ...
    action do_recirculate(){
    	...
    	action port_2(bit<7> port_no){
        	ig_tm_md.ucast_egress_port[8:7] = 1;
        	ig_tm_md.ucast_egress_port[6:0]= port_no;

    	}
        action port(bit<7> port_no){

        	ig_tm_md.ucast_egress_port[8:7] = 3;
        	ig_tm_md.ucast_egress_port[6:0]= port_no;
    ...
    apply {
        if(ig_intr_md.ingress_port[6:0]==56){
            drop();
        }
        ...
        if(hdr.ascon.curr_round==0x24){
            ...
            ig_tm_md.ucast_egress_port[8:7] = 2;
