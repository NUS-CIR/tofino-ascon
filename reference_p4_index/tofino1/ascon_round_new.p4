    ...
    // @in_hash{meta.t0[63:32] = meta.t0[63:32] ^ (~meta.t0[63:32] & meta.t0[63:32]);}
    // @in_hash{meta.t0[31:0] = meta.t0[31:0] ^ (~meta.t0[31:0] & meta.t0[31:0]);}
    // @in_hash{meta.t1 = hdr.ascon.s1 ^ (~hdr.ascon.s2 & hdr.ascon.s3);}
    // @in_hash{meta.t2 = hdr.ascon.s2 ^ (~hdr.ascon.s3 & hdr.ascon.s4);}
    // @in_hash{meta.t3 = hdr.ascon.s3 ^ (~hdr.ascon.s4 & hdr.ascon.s0);}
    // @in_hash{meta.t4 = hdr.ascon.s4 ^ (~hdr.ascon.s0 & hdr.ascon.s1);}
    // @in_hash{ meta.t1 = (hdr.ascon.s1 ^ (~hdr.ascon.s2 & hdr.ascon.s3)) ^ (hdr.ascon.s0 ^ (~hdr.ascon.s1 & hdr.ascon.s2));}
    // @in_hash{ meta.t0 = (hdr.ascon.s0 ^ (~hdr.ascon.s1 & hdr.ascon.s2)) ^ (hdr.ascon.s4 ^ (~hdr.ascon.s0 & hdr.ascon.s1));}
    // @in_hash{ meta.t3 = (hdr.ascon.s3 ^ (~hdr.ascon.s4 & hdr.ascon.s0)) ^ (hdr.ascon.s2 ^ (~hdr.ascon.s3 & hdr.ascon.s4));}
    // @in_hash{ meta.t2 = ~(hdr.ascon.s2 ^ (~hdr.ascon.s3 & hdr.ascon.s4));}
...
//for hdr.s0
    @in_hash{hdr.ascon.s0[63:32] = meta.t0[63:32] & (meta.t0[18:0] ++ meta.t0[63:51]) ;}
